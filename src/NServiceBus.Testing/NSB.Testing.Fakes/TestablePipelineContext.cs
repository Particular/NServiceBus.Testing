...
#pragma warning restore PS0002 // Instance methods on types implementing ICancellableContext should not have a CancellationToken parameter
    ...
    /// <param name="options">The options for the send.</param>
    public virtual Task Send<T>(Action<T> messageConstructor, SendOptions options) => Send(messageCreator.CreateInstance(messageConstructor), options);
    ...
    public virtual Task Publish<T>(Action<T> messageConstructor, PublishOptions publishOptions) => Publish(messageCreator.CreateInstance(messageConstructor), publishOptions);
