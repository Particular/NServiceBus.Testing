...
public class TimeoutMessage<TMessage> : OutgoingMessage<TMessage, SendOptions>
{
    ...
    /// </summary>
    public TimeoutMessage(TMessage message, SendOptions options, TimeSpan within) : base(message, options) => Within = within;

    /// <summary>
    /// Creates a new instance for the given message and options.
    /// </summary>
    public TimeoutMessage(TMessage message, SendOptions options, DateTimeOffset at) : base(message, options) => At = at;
